...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($name && $serial_no && $model_no && $main_category && $category && $location && $type && $qty && $price && $total_price && $item_remarks && $project_name && $project_no && $purchase_order && $invoice_no && $vendor_name && $date && $place && $mos && $receiver_name && $vendor_remarks) {
        ...
        $checkSql = "SELECT * FROM $main_category WHERE model_no = ? OR serial_no = ?";
        $checkStmt = $conn->prepare($checkSql);
        $checkStmt->bind_param("ss", $model_no, $serial_no);
        $checkStmt->execute();
        ...
        if ($checkResult->num_rows > 0) {
            ...
            echo json_encode(['status' => 'Failed', 'remark' => 'Model No or Serial No already available']);
        } else {

            $selectedVendor = $vendor_name;

            // Model number does not exist, proceed to save data
            $sql = "INSERT INTO $main_category (name, serial_no, model_no, main_category, category, location, type, qty, item_remarks, project_name, project_no, purchase_order, invoice_no, vendor_name, date, place, mos, receiver_name, vendor_remarks, Stock_in_out, barcode, price, total_price) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("sssssssssssssssssssssss", $name, $serial_no, $model_no, $main_category, $category, $location, $type, $qty, $item_remarks, $project_name, $project_no, $purchase_order, $invoice_no, $selectedVendor, $date, $place, $mos, $receiver_name, $vendor_remarks, $Stock_in_out, $barcodeData, $price, $total_price);
